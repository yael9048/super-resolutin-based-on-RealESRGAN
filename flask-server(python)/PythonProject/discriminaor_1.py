...
class UNetDiscriminatorSN(nn.Module):
  def __init__(self,num_in_ch, num_feat=64, skip_connection=True):
    ...
    #שלושת השכבות הבאות מבצעות הגדלה לתמונה
    self.conv4 = norm(nn.Conv2d(num_feat * 8, num_feat * 4, 3, 1, 1, bias=False))
    self.conv5 = norm(nn.Conv2d(num_feat * 4, num_feat * 2, 3, 1, 1, bias=False))
    self.conv6 = norm(nn.Conv2d(num_feat * 2, num_feat, 3, 1, 1, bias=False))

    #שכבות נוספות להפקת מפת הציונים הסופית
  ...
  def forward(self,x):
   #נבצע הקטנה
    ...
    # align_corners=False  לא משאירים את הפיקסלים המקוריים בפינות אלא מחשבים את היחס שלהם בתמונה השלימה
    ...
    #לא מבצעים אקטיבציה בשכבה האחרונה כדי לשמור על המידע גולמי שייכנס לLOSS
    out = self.conv9(out)
